...
import github.qingha.io.FileProperty;
import github.qingha.io.ProgressReceiver;
import github.qingha.util.MismatchingVersionException;
import github.qingha.util.MtrVersion;
...
public class RemoteMetadata {
    ...
    public byte[] fetchDirChecksum(ProgressReceiver cb) throws Exception {
        ...
        if (metaString.startsWith("{")) {
            JsonObject metadataObj = ResourceSynchronizationClient.JSON_PARSER.parse(metaString).getAsJsonObject();
    ...
    public void fetch(ProgressReceiver cb) throws Exception {
        dirs.clear();
        files.clear();
        var metadataObj = ResourceSynchronizationClient.JSON_PARSER.parse(
    ...
    public void assertMetadataVersion(JsonObject metadataObj) throws MismatchingVersionException {
        if (metadataObj.has("client_version")) {
            ...
            if (!MtrVersion.parse(ResourceSynchronizationClient.MOD_VERSION).matches(requestedVer)) {
                throw new MismatchingVersionException(requestedVer, ResourceSynchronizationClient.MOD_VERSION);
            }
        }
        int metadataVersion = 1;
        if (metadataObj.has("version")) metadataVersion = metadataObj.get("version").getAsInt();
