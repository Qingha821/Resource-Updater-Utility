                ...
                while (true) {
                    try {
                        ...
                        if (task.failedAttempts > 0) {
                            delayedProgresses.add(() -> progressReceiver.printLog(String.format("Downloading files ... (Retry %d succeed)",
                                    task.failedAttempts)));
                        }
                        break;
                    } catch (Exception ex) {
                        ...
                        if (task.failedAttempts < MAX_RETRIES) {
                            delayedProgresses.add(() -> {
                                ...
                                progressReceiver.printLog(String.format("Retry %d: %s", task.failedAttempts, ex));
                            });
                        } else {
                            throw ex;
                        }
