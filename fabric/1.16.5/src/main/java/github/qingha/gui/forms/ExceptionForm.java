...
public class ExceptionForm implements GlScreenForm {
    ...
    public boolean shouldStopPausing() {
        ...
        if (InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_HOME)) {
            logViewOffset = 0;
        } else if (InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_END)) {
            logViewOffset = maxLogViewOffset;
        } else if (InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_PAGE_UP)) {
            logViewOffset = Math.max(0, logViewOffset - logLines);
        } else if (InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_PAGE_DOWN)) {
            logViewOffset = Math.min(maxLogViewOffset, logViewOffset + logLines);
        } else if (InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_UP)) {
            logViewOffset = Math.max(0, logViewOffset - 1);
        } else if (InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_DOWN)) {
            logViewOffset = Math.min(maxLogViewOffset, logViewOffset + 1);
        }

        return InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_ENTER);
    }
