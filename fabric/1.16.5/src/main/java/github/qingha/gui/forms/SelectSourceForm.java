...
public class SelectSourceForm implements GlScreenForm {
    ...
    public boolean shouldStopPausing() {
        ...
        if (InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_UP) || InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_W)) {
            if (heldKey != GLFW.GLFW_KEY_UP && heldKey != GLFW.GLFW_KEY_W) {
                selectedIndex = Math.max(0, selectedIndex - 1);
                heldKey = GLFW.GLFW_KEY_UP;
            }
        } else if (InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_DOWN) || InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_S)) {
            if (heldKey != GLFW.GLFW_KEY_DOWN && heldKey != GLFW.GLFW_KEY_S) {
                selectedIndex = Math.min(ResourceSynchronizationClient.CONFIG.sourceList.value.size(), selectedIndex + 1);
                heldKey = GLFW.GLFW_KEY_DOWN;
            }
        } else if (InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_ENTER)
            || InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_SPACE)
            || InputConstants.isKeyDown(glfwWindow, GLFW.GLFW_KEY_RIGHT)
            || countdownExpired) {
