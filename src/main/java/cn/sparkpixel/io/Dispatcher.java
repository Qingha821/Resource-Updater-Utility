...
import cn.sparkpixel.io.network.PackOutputStream;
import cn.sparkpixel.io.network.RemoteMetadata;
...
public class Dispatcher {
    ...
    public boolean runSync(String baseDir, Config.SourceProperty source, ProgressReceiver cb) throws Exception {
        try {
            ...
            cb.printLog("Resource Pack Updater v" + ResourceUpdaterUtility.MOD_VERSION + " (C) Zbx1425, www.zbx1425.cn");
...
//            cb.printLog("Target: " + baseDir);
    ...
    public String fetchModsList(String baseUrl) throws IOException {
        String modsMetaUrl = baseUrl + "/modsmeta.json";
        HttpRequest request = HttpRequest.newBuilder()
                .uri(URI.create(modsMetaUrl))
                .header("Accept", "application/json")
                .GET()
                .build();

        try {
            HttpResponse<String> response = ResourceUpdaterUtilityClient.HTTP_CLIENT
                    .send(request, HttpResponse.BodyHandlers.ofString());

            if (response.statusCode() == 200) {
                return response.body();
            } else {
                throw new IOException("Failed to get mods list: HTTP " + response.statusCode());
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new IOException("Getting mods list was terminated", e);
        }
    }
}
