...
import cn.sparkpixel.io.FileProperty;
import cn.sparkpixel.io.ProgressReceiver;
import cn.sparkpixel.util.MismatchingVersionException;
import cn.sparkpixel.util.MtrVersion;
...
public class RemoteMetadata {
    ...
    public byte[] fetchDirChecksum(ProgressReceiver cb) throws Exception {
        ...
        if (metaString.startsWith("{")) {
            JsonObject metadataObj = ResourceUpdaterUtility.JSON_PARSER.parse(metaString).getAsJsonObject();
    ...
    public void fetch(ProgressReceiver cb) throws Exception {
        dirs.clear();
        files.clear();
        var metadataObj = ResourceUpdaterUtility.JSON_PARSER.parse(
    ...
    public void assertMetadataVersion(JsonObject metadataObj) throws MismatchingVersionException {
        if (metadataObj.has("client_version")) {
            ...
            if (!MtrVersion.parse(ResourceUpdaterUtility.MOD_VERSION).matches(requestedVer)) {
                throw new MismatchingVersionException(requestedVer, ResourceUpdaterUtility.MOD_VERSION);
            }
        }
        int metadataVersion = 1;
        if (metadataObj.has("version")) metadataVersion = metadataObj.get("version").getAsInt();
